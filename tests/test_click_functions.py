...
class TestClickFunctions(unittest.TestCase):
  ...
  def test_validate_click(self):    
    ...
    data = json.loads(message)
    result = click.validate_click(data['uuid'], data['user_uuid'], data['button_uuid'], data.get('url', None), data['referrer_user_uuid'])
    self.assertTupleEqual((568334, None, 702273458, None, None, 659867728, 'mrjingles', None), result)
    ...
    data = json.loads(message)
    result = click.validate_click(data['uuid'], data['user_uuid'], data['button_uuid'], data.get('url', None), data['referrer_user_uuid'])
    self.assertTupleEqual((568334, None, 702273458, None, 755095536, 659867728, 'mrjingles', None), result)
    ...
    data = json.loads(message)
    result = click.validate_click(data['uuid'], data['user_uuid'], data['button_uuid'], data.get('url', None), data['referrer_user_uuid'])
    self.assertTupleEqual((568334, None, 749341768, None, None, 1005146552, 'thisisfrancis'), result[:-1])
