...
class TestClickFunctions(unittest.TestCase):
  ...
  def test_validate_click(self):    
    ...
    data = json.loads(message)
    result = click.validate_click(data['uuid'], data['user_uuid'], data['button_uuid'], data.get('url', None), data.get('comment_uuid', None), data['referrer_user_uuid'])
    self.assertTupleEqual((568334, None, 702273458, None, None, None, 659867728, 'mrjingles', None), result)
    ...
    data = json.loads(message)
    result = click.validate_click(data['uuid'], data['user_uuid'], data['button_uuid'], data.get('url', None), data.get('comment_uuid', None), data['referrer_user_uuid'])
    self.assertTupleEqual((568334, None, 702273458, None, None, 755095536, 659867728, 'mrjingles', None), result)
    ...
    data = json.loads(message)
    result = click.validate_click(data['uuid'], data['user_uuid'], data['button_uuid'], data.get('url', None), data.get('comment_uuid', None), data['referrer_user_uuid'])
    self.assertTupleEqual((568334, None, 749341768, None, None, None, 1005146552, 'thisisfrancis'), result[:-1])
    self.assertEqual(659867728, result[8][0]['user']) 
    self.assertEqual(10, result[8][0]['share_amount']) 
    ...
    self.assertTupleEqual((None, None, 659867728, 'mrjingles', None), result[4:])
